 ...
 * to the homepage. A valid HMAC proves the lead/touch pair came from IGY, then the
 * server resolves campaign identity from the lead row and records an opaque
 * attribution session. The visitor still lands on the normal homepage; pricing,
 * signup, consent and eligibility are unchanged.
 */
export async function GET(req: Request) {
  ...
  // Never expose whether a lead/token exists. Invalid entry simply becomes an
  // ordinary homepage visit and carries no attribution.
  if (!touch || !language || !verifyOutreachAttributionToken(leadId, touch, language, token)) {

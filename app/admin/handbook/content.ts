    ...
    entries: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: "callout", kind: "info", title: "Provider failover", text: "Discovery runs on OpenAI. If OpenAI runs out of API credits mid-run, the current search lane automatically retries on Anthropic (Claude) instead of stalling the whole run, and you get one ops-alert email (further failovers in the next few hours are logged but not re-emailed). This needs ANTHROPIC_API_KEY configured; without it, discovery is OpenAI-only and a credit-out pauses it until you top up. It is failover only, not cost routing: it reacts to a real credit-exhaustion error, never to ordinary rate limits, and does not predict balances." },

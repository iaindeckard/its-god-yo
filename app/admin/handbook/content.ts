...
export const HANDBOOK_UPDATED = "August 24, 2026";
  ...
  // ------------------------------------------------------------------ PART 4
  {
    ...
    entries: [
      ...
      },
      {
        ...
        title: "How changes to IGY get made",
        blocks: [
          { type: "prose", text: "IGY's code is changed carefully and on purpose. A few standing rules govern how work lands, and they held up repeatedly during the 2026-08-24 outreach work." },
          { type: "list", items: [
            "A scoped plan comes before code. Non-trivial changes are investigated and written up as a plan for Iain to review before anything is edited. Investigate first, build second is the default, not the exception.",
            "Completion is verified against live systems, not asserted. A change is treated as done only after it is checked directly against the real systems it touches (Supabase, Stripe, Vercel). A plausible-sounding “it's done” has needed correction more than once, so a claim is not trusted until it is confirmed.",
            "Live promo codes are Iain's own action. Production discount codes are created by Iain personally in the Promo Code Studio, never by an automated tool or the coding assistant (see Part 5).",
          ] },
        ],
      },
      {
    ...
    entries: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: "callout", kind: "warning", title: "Live campaign codes are created by Iain personally", text: "The public campaign codes (APPRECIATION10, TOUCH2-25, and the affinity codes) are always created by Iain personally, here in the Studio, never generated automatically and never by the coding assistant. They move real money through live Stripe, so creating one is a deliberate human action. The email copy that references a code is only wired up in code after the code already exists." },
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: "callout", kind: "info", title: "Provider failover", text: "Discovery uses two providers, OpenAI and Anthropic (Claude). One is the primary and the other is held in reserve; if the primary runs out of API credits mid-run, the current search lane automatically retries on the reserve instead of stalling the whole run, and you get one ops-alert email (further failovers in the next few hours are logged but not re-emailed). The order is set by OUTREACH_DISCOVERY_PRIMARY (default: OpenAI primary, Anthropic reserve); it is a global switch, not per-campaign, so remember to set it back after a temporary flip. Both providers' keys must be configured for failover to work: OPENAI_API_KEY, and (added 2026-08-24) ANTHROPIC_API_KEY in the Vercel Production and Preview environments. OpenAI runs in background mode; Anthropic runs synchronously and uses adaptive thinking on heavy prompts. It is failover only, not cost routing: it reacts to a real credit-exhaustion error, never to ordinary rate limits, and does not predict balances." },
          { type: "subheading", text: "Lead verification (before anything can send)" },
          { type: "prose", text: "Before a lead can receive a live email it must pass an automated verification check: one of its cited source pages has to actually mention the organization and something youth-ministry-related, and its contact email's domain has to accept mail (a valid address whose domain has a working mail record). Pass both and the lead is marked verified; if either check can't be confirmed, the lead is held as “needs manual review,” never sent automatically. Verification is re-run at discovery and can be re-run by hand from the campaign screen, and it expires after 90 days so a long-staged lead is re-checked before it can go out." },
          { type: "callout", kind: "danger", title: "Verification is a hard gate in code, not a checklist", text: "The send job itself refuses to email any lead that isn't verified (or manually overridden) and current. An unverified, stale, or needs-manual lead is skipped in BOTH the live send and the dry-run preview, so what you preview is exactly what would send. There is no request setting that bypasses this." },
          { type: "callout", kind: "info", title: "Permission: marketing.outreach.verify_override", text: "Clearing a lead that failed automated verification (marking it manually verified so it can send) requires marketing.outreach.verify_override, a separate and higher bar than running verification (which uses marketing.outreach.manage). It is a real access change: it lets a person push a lead past the automated safety check, so use it only for a genuine false negative (for example a real church whose site couldn't be fetched). Default: super_admin only; assignable to other roles in Roles & staff like any other permission." },
          { type: "subheading", text: "Promoting leads to send" },
          { type: "prose", text: "You choose the exact freshly verified staged leads that become active. Held or stale leads cannot be selected, and the server independently enforces that rule. Only promoted (active) leads can ever receive an email. This is the deliberate gate between “found” and “contacted.”" },
          { type: "subheading", text: "Sending" },
          { type: "prose", text: "A campaign can be sent as its own deliberate push, separate from the company-wide send, so its results trace cleanly to that campaign. You select the exact active recipients before previewing or scheduling; that immutable list becomes the release snapshot. Every send is still governed by the same outreach approvals (copy approved, legal approved, and the live master switch) plus an optional address allowlist. If an allowlist or verification gate excludes anyone in a scheduled snapshot, the campaign pauses instead of silently advancing its cadence." },
          { type: "callout", kind: "danger", title: "The three approvals are global, not per-campaign", text: "OUTREACH_COPY_APPROVED, OUTREACH_LEGAL_APPROVED, and OUTREACH_SEND_LIVE are the approvals that arm a live send, and all three are GLOBAL environment flags, not per-campaign settings. Opening them arms every eligible campaign at once, not just the one you are looking at. Treat flipping any of them as arming all outreach." },
          { type: "callout", kind: "warning", title: "You cannot read these flags back once set", text: "All three approval flags are marked Sensitive in Vercel. Once set, their value can never be read back by anyone, not through the CLI, the API, or the dashboard; they can only be overwritten. Do not waste time trying to confirm a value that way. Track the intended state in the authoritative outreach state doc instead. (OUTREACH_SEND_ALLOWLIST is plaintext, not Sensitive, so it can be read back.)" },
          { type: "callout", kind: "info", title: "How the allowlist actually works", text: "OUTREACH_SEND_ALLOWLIST restricts a live send to a specific set of addresses. It applies ONLY at the live send, never during a dry-run: a dry-run always renders the full eligible set regardless of the allowlist, which is expected, not a bug. Use it to hold a first controlled batch to known addresses, and leave it empty for an unrestricted send." },
          { type: "callout", kind: "info", title: "How releases actually run (the real cron cadence)", text: "There is no monthly outreach-send cron. The real release worker, /api/cron/outreach-send, runs every 15 minutes and releases each campaign the moment its own release_at comes due, so a scheduled campaign goes out on the next 15-minute tick after its release time, not on a monthly cycle. Discovery (/api/cron/outreach-discovery) runs monthly on the 1st, and reinvestment (/api/cron/outreach-reinvestment) runs weekly on Monday. The send gate has been open since 2026-08-12 (following an explicit 2026-08-10 decision to launch full outreach) and has stayed open; that open state is expected, not an incident, so check the authoritative outreach state doc before investigating it as one." },
          { type: "subheading", text: "Per-campaign offer" },
          { type: "prose", text: "Each campaign can carry its own discount and message variant, so you can tell a weak region apart from a weak offer. That discount number drives the first-touch wording where a variant shows a percent (for example the Catholic School pitch's “10% off”), and the message copy itself is fixed in code and approved, so a campaign can only pick an approved variant, never write its own. The church follow-up (second touch) does NOT use this per-campaign number; it carries one flat shared code for every campaign, described next." },
          { type: "subheading", text: "The two-touch follow-up sequence" },
          { type: "prose", text: "Most church campaigns are a two-touch sequence: a code-free introduction, then a single follow-up about 30 days later if the church hasn't signed anyone up. IGY stops after two touches; there is no endless re-sending." },
          { type: "list", items: [
            "Church first touch: a plain introduction with no code, just a link to see how IGY works.",
            "Church second touch (about 30 days later): the follow-up that carries the offer. It embeds one shared flat code, TOUCH2-25 (25% off an individual monthly or annual, family, or gift subscription; once per customer; first-time customers only; valid through October 23, 2026).",
            "One flat code for everyone: TOUCH2-25 is the same for every church campaign regardless of that campaign's original first-touch discount tier. It is deliberately NOT a per-campaign proportional bump, so a 15%-off campaign and a 20%-off campaign both land at a flat 25% on the follow-up. One code, created once, reused everywhere.",
            "No per-lead codes anymore: the follow-up no longer mints a unique code per church. Because the code is shared it is not stored on each lead, so per-lead attribution for a follow-up conversion rides the signed signup link in the email instead of a promo-code match.",
          ] },
          { type: "callout", kind: "info", title: "Reactive follow-up timing", text: "The second touch is scheduled reactively: its release time is set at the moment the first touch actually finishes sending, not pre-calculated from when the campaign was approved or scheduled. This closed a real drift bug where a delayed first send (for example while a gate was closed) had left the follow-up pegged to a stale time. In practice the follow-up fires about 30 days after the first email truly went out." },
          { type: "callout", kind: "info", title: "Two ops alerts on a live wave", text: "Two email alerts now watch a live send (the same sendOpsAlert path to Resend to the owner's inbox, deduped so each fires once per campaign). A bounce-rate alert fires if a campaign's hard-bounce rate crosses about 15% (after at least 20 sends, so a tiny early sample can't trip it), to catch a bad lead list early. And a confirmation fires when a Touch-2 wave actually sends, so a real send is confirmed rather than assumed." },
          { type: "subheading", text: "Catholic K-12 Schools campaigns" },
          { type: "prose", text: "The “Catholic School” message variant runs a national campaign to Catholic K-12 schools, one campaign per state, instead of the default church/youth-ministry outreach. It behaves differently on purpose, and those differences are fixed in code, not per-campaign settings." },
          { type: "list", items: [
            "School discovery, not church discovery: a school campaign searches diocesan school lists first (the Wikipedia “List of schools in the Roman Catholic (Arch)diocese of …” pages), then each diocese's official school directory and the NCEA locator, extracting each school's public office or admissions email. It does NOT look for a “youth ministry,” and leads are marked as schools.",
            "School-aware verification: because schools have no youth ministry, a school lead is verified against school evidence (grade levels, admissions, enrollment) rather than youth-ministry keywords; the email/mail-record check is unchanged.",
            "First touch is the pitch, with the code up front: unlike the church sequence (a code-free intro), a school's first email is the pitch itself and already contains the code and the DM from Him upsell.",
            "Second touch is a distribution ask, added 2026-08-24: this campaign was originally single-touch, but now a school that hasn't signed anyone up gets one follow-up about 30 days later. It is a code-free distribution ask, not a repeat pitch or a bigger discount. It asks the school to share the existing offer with families, includes a ready-to-paste blurb for their newsletter or bulletin, and asks who the right contact is if the office isn't who handles that, so a wrong-contact problem surfaces instead of going nowhere a second time. No new code is offered; APPRECIATION10 stays the only school code.",
            "One shared code (APPRECIATION10), not a minted per-lead code: every school shares the same public code (10% off, valid through Dec 31 2026, no DM-from-Him requirement, with a short student/caregiver/faculty attestation at checkout). Because the code is shared, it is deliberately not stored on each lead, so a single redemption can never mark other schools converted; per-school attribution rides the signed signup link in the email instead.",
            "Catholic High School, New Iberia (chspanthers.com) is hard-excluded from school discovery and any school send. It has its own separate code and was contacted personally.",
          ] },
          { type: "callout", kind: "info", title: "Same send gate as every campaign", text: "A school campaign is still fully governed by the outreach approvals (copy approved, legal approved, live master switch) and the per-lead verification gate. Building the campaign and running discovery never sends anything; a live send requires all the same sign-offs." },
          ...
          { type: "subheading", text: "ROI reinvestment" },
          { type: "prose", text: "Record the actual campaign investment on the performance screen. ROI is calculated from realized net revenue after Stripe fees, refunds, and chargebacks: (net revenue minus investment) divided by investment. A campaign with no recorded investment is excluded rather than treated as free or infinitely profitable." },
          { type: "list", items: [
            "Every Monday the system evaluates campaigns and creates one capped proposal when the configured sample, conversion, and ROI thresholds are met.",
            "The proposal freezes the policy, source evidence, ROI, and proposed dollars so later revenue changes cannot rewrite the approval record.",
            "Approval requires marketing.outreach.reinvestment.approve and creates budgeted draft campaigns in the winning markets. It does not discover churches, promote recipients, schedule, send, or spend money through an external provider.",
            "Pause automatic proposals at any time. Rejected proposals remain in the audit history.",
          ] },
          { type: "callout", kind: "info", title: "Permission: marketing.outreach.view", text: "Viewing requires marketing.outreach.view; creating campaigns, promoting leads, running verification, and sending require marketing.outreach.manage; manually overriding a verification result requires marketing.outreach.verify_override. All three default to super_admin only." },
        ],
      },
      {
        id: "admin-roles",

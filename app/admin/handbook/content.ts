...
// before publishing. Mirrors USN's /admin/handbook implementation.
//
...
export const PARTS: Part[] = [
  // ------------------------------------------------------------------ START HERE
  {
    ...
    entries: [
      ...
      {
        id: "company-context",
        title: "Company context: DEI, USN, and IGY",
        kicker: "Start here",
        blocks: [
          { type: "prose", text: "IGY is a product of Deckard Enterprise International, LLC (DEI), a holding company that also owns USN (Unfiltered Sports Network). DEI handles licensing, overhead, and revenue-share; each venture pays DEI a cut." },
          { type: "callout", kind: "warning", title: "Sister products, not shared infrastructure", text: "IGY and USN are separate Supabase projects, separate Stripe accounts, and separate codebases. If you move between the two, do not assume shared logins, shared data, or shared Stripe/Supabase access — there isn't any." },
        ],
      },
    ],
  },
    ...
    entries: [
      {
        ...
        blocks: [
          ...
          { type: "subheading", text: "The value we carry over from USN's playbook" },
          ...
          { type: "subheading", text: "Where IGY sits relative to USN" },
          { type: "prose", text: "Same parent company (DEI), same operating discipline — but a completely separate product, market, and infrastructure. Treat them as sister products, never as one system." },
        ],
      },
    ],
  },
    ...
    entries: [
      {
        ...
        blocks: [
          ...
          { type: "table", head: ["Plan", "Price", "Notes"], rows: [
            ...
            ["Group / church", "$28–$36 per teen / year", "Per-teen quantity in bands: 1–50 = $28, 51–150 = $32, 151–300 = $36. 301+ is contact-us (no self-serve price)."],
          ] },
  ...
  // ------------------------------------------------------------------ PART 3
  {
    ...
    number: "3",
    title: "Programs",
    blurb: "Cornerstone (live), the Sponsor Program (paused), and the affinity promo codes.",
    entries: [
      {
      ...
      {
        id: "sponsor-program",
        title: "Sponsor Program (paused, not dead)",
        blocks: [
          { type: "prose", text: "The Sponsor Program was live around 2026-07-22, drew zero sponsors and zero inquiries, and was paused in favor of Cornerstone. Its public surfaces are hidden but the data and schema are intact." },
          { type: "callout", kind: "danger", title: "Say “paused,” not “cancelled”", text: "Don't tell anyone the Sponsor Program is cancelled — it's paused, and could come back." },
        ],
      },
      {
        id: "promo-codes",
    ...
    entries: [
      {
        ...
        blocks: [
          { type: "callout", kind: "danger", title: "A git push does NOT ship IGY", text: "Unlike USN (which deploys straight to usn.network), IGY does NOT auto-deploy on git push. A push only records the commit — someone has to run `vercel --prod --yes` to actually ship it. If you skip this, you'll think something shipped when it didn't." },
        ],
      },
      {
        id: "dei-rollup",
        ...
        blocks: [
          { type: "prose", text: "IGY reports into a separate DEI-owned Supabase project via a nightly batch ETL. DEI never live-queries IGY's database directly, and IGY never live-queries USN's. The flow is read-only, one-way, and runs on a cron." },
        ],
      },
      {
  ...
  // ------------------------------------------------------------------ PART 5 (role-gated by relatedRoute)
  {
    ...
    entries: [
      ...
      },
      {
        ...
        blocks: [
          { type: "prose", text: "Where daily-verse content is approved before it can reach subscribers — the human gate on the content pipeline." },
          { type: "list", items: [
      ...
      },
      {
        ...
        blocks: [
          { type: "prose", text: "The admin surface for the Sponsor Program. The public program is paused (Part 3), but the admin screen and data remain intact so it can be reactivated." },
          { type: "list", items: [
            "View sponsors (marketing.sponsors.view).",
            "Add/edit/vet/expire sponsors (marketing.sponsors.manage).",
          ] },
          { type: "callout", kind: "warning", title: "Permission: marketing.sponsors.view (super_admin)", text: "The screen exists even though the public program is paused — don't take its presence as a sign the program is live." },
        ],
      },
      {
    ...
    entries: [
      {
        ...
        blocks: [
          { type: "callout", kind: "escalate", title: "1 · Customer safety concern", text: "If something in a support message suggests a subscriber may be at risk — Iain, directly, immediately. This is the one category treated as urgent-response, no exceptions." },
          ...
          { type: "callout", kind: "escalate", title: "4 · Security incident", text: "Breach, leaked credential, and the like — Iain, directly, immediately. Same urgency tier as a customer safety concern." },
        ],
      },
      {
        id: "what-not-to-do",
        ...
        blocks: [
          { type: "list", items: [
            "Don't discuss unreleased features (Spanish, the paused Sponsor Program) as if they're live.",
    ...
    entries: [
      {
        ...
        blocks: [
          { type: "table", head: ["Term", "Meaning"], rows: [
            ...
            ["USN", "Unfiltered Sports Network — IGY's sister product under DEI."],

  ...
  return (
    <div className="admin-shell">
      <aside className="admin-side">
        ...
        <div style={{ fontSize: 12, color: "#7f9cc4", letterSpacing: "0.08em", textTransform: "uppercase" }}>Admin</div>
        <nav className="admin-nav">
          {nav.length === 0 && <span style={{ fontSize: 13, color: "#7f9cc4", padding: "8px 12px" }}>No sections available for this role.</span>}
          {nav.map((n) => (
            <Link key={n.href} href={n.href}>{n.label}</Link>
          ))}
        </nav>
        <div style={{ marginTop: "auto", paddingTop: 24, fontSize: 12, color: "#7f9cc4" }}>
          <div>Acting role</div>
          <div style={{ color: "#cdd9ea", fontWeight: 700 }}>{staff.jobRole}</div>
        </div>
      </aside>

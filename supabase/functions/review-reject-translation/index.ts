...
Deno.serve(async (req: Request) => {
  ...
  const { daily_slot_id, reviewer_id, corrected_translation, reason } = body;
  if (!daily_slot_id || !reviewer_id || !corrected_translation || !reason) {
    return json(400, { error: "daily_slot_id, reviewer_id, corrected_translation, and reason are all required" });
  }

...
const SG_DISCLOSURE: Record<string, (n: string) => string> = {
  en: (n) => `${n} will get a text asking them to confirm, and we cannot start their gift without their own OK. If they do not reply, we will send one reminder about a week later. If they still have not confirmed after about 30 days, the gift converts to It's God, Yo! account credit for the purchaser instead. We never keep texting someone who has not responded.`,
  es: (n) => `${n} recibirá un mensaje pidiéndole que confirme, y no podemos iniciar su regalo sin su propio consentimiento. Si no responde, enviaremos un recordatorio aproximadamente una semana después. Si aún no ha confirmado después de unos 30 días, el regalo se convierte en crédito de cuenta de It's God, Yo! para el comprador. Nunca seguimos enviando mensajes a alguien que no ha respondido.`,
};
